export default async function handler(request, response) {
  ...
  try {
    ...
    if (period === "today") {
      ...
      startDate = new Date(
        japanMidnight.getTime() - 9 * 60 * 60 * 1000
      );
    }
    ...
    params.set(
      "select",
      "content_id,title,actress,clicked_at"
    );
    ...
    for (const row of rows) {
      const actressText = String(row.actress || "").trim();

      if (!actressText) {
        continue;
      }

      const actressNames = actressText
        .split(/[、,／/]/)
        .map((name) => name.trim())
        .filter(Boolean);

      const uniqueNames = [...new Set(actressNames)];

      for (const actressName of uniqueNames) {
        const current = actressMap.get(actressName);

        if (current) {
          current.clicks += 1;
          current.products.add(String(row.content_id || ""));
    ...
    const rankings = Array.from(actressMap.values())
      .map((item) => ({
        actress: item.actress,
        clicks: item.clicks,
        product_count: Array.from(item.products).filter(Boolean).length,
    ...
    return response.status(200).json({
      ...
      total_actresses: actressMap.size,
      rankings,
    });
  } catch (error) {
    console.error(error);
